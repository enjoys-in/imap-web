...
interface WizardContextValue {
  ...
  onCompleteView?: React.ReactNode
}
...
function Wizard({ initialStep = 0, totalSteps, children, className,onCompleteView }: WizardProps) {
  ...
  return (
    <WizardContext.Provider value={value}>  
      <div className={cn("wizard-container", className)}>{isComplete ? (onCompleteView || <WizardThankYou />) : children}</div>
