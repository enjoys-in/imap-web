...
export const MailDisplay = async ({ folder, uid, message_id, result }: MailDisplayProps) => {
    try {
        const parsedMail = await simpleParser(result);
        ...
        return (
            ...
            >
                <h2 className="pl-4 text-2xl font-bold mt-4">{parsedMail?.subject}</h2>
              ...
              <AvatarFallback className='flex items-center justify-center h-8 w-8 bg-muted-foreground/50 dark:bg-muted/50 hover:rounded-xl rounded-full'>
                  ...
                  .join("").toUpperCase()}
                        ...
                        <div className="flex flex-col ml-2 md:ml-0">
                            <div className="flex flex-col sm:flex-row gap-2 align-text-bottom">
                                <div className="font-semibold">{parsedMail?.from?.text && formatEmail(parsedMail?.from.text)}</div>
                            </div>
                                        ...
                                        <PopoverContent className="flex w-[450px] px-4 sm:px-6 md:px-8 lg:px-10">
                                            <div className="flex flex-row gap-4 px-2 py-4 text-sm">
                                                ...
                                                <div className="flex flex-col">
                                                    <div>
                                                        <strong>{parsedMail?.from?.text && formatEmail(parsedMail?.from?.text)}</strong>

                                                    </div>
                                                    {/* <div>{parsedMail?.to?.map((to) => formatEmail(to.text))}</div>
                          <div>{moment(parsedMail?.date).format("lll")}</div>
                          <div>{parsedMail?.subject}</div>
                          <div>{parsedMail?.to?.map((to) => formatEmail(to.text))}</div> */}
                    ...
                    <div className="w-full lg:w-auto flex justify-end">
                        ...
                        <Tooltip>
                            <TooltipTrigger asChild>
                                <Button variant="ghost" size="icon" disabled={!parsedMail}>
                ...
                <MailDisplay2
                    ...
                    emailData={{
                        ...
                        receivedOn: parsedMail?.date?.toDateString(),
                        email: parsedMail?.from?.text,
                        subject: parsedMail?.subject,
                        decodedBody: parsedMail?.html || parsedMail?.text
